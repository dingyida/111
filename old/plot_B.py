...
EPS           = 1e-6
penalty_coef  = 1e3
large_change_threshold = 0.15
recompute_hessian_every = 20
full_slsqp_period = 20          # run full SLSQP every N frames
...
class IncrementalOptimizer:
    ...
    def update(self, A_mat, g):
        ...
        try:
            ...
            self.initialize(A_mat, g)
            return self.lam
        self.lam = self._project(self.lam + d[:N])
        self.nu += d[N]
        self.count += 1
        self.prev_A, self.prev_g = A_mat.copy(), g.copy()
        return self.lam
...
if __name__ == "__main__":
    ...
    # containers: dict[Lambda_max] -> list of AoI values
    AoI_inc_all  = {}
    AoI_unif_all = {}
    AoI_full_all = {}

    for Lmax in Lambda_list:
        inc_opt     = IncrementalOptimizer(mu, Lmax)
        lam_uniform = np.ones(num_cams) * (Lmax / num_cams)
        ...
        for fp, t in zip(files, frames):
            ...
            print(f"[Λ={Lmax:>.1f}] Frame {t}: inc={val_inc:.4f}, unif={val_unif:.4f}, full={val_full:.4f}")

        AoI_inc_all[Lmax]  = AoI_inc
        AoI_unif_all[Lmax] = AoI_u
        AoI_full_all[Lmax] = AoI_full

    # ---- trimmed mean helper ----
    def trimmed_mean(arr, k=5):
        if len(arr) <= 2*k:
            # fallback: just mean if not enough samples
            return float(np.mean(arr))
        s = np.sort(arr)
        return float(np.mean(s[k:-k]))

    # compute trimmed means
    tm_inc  = [trimmed_mean(AoI_inc_all[L])  for L in Lambda_list]
    tm_unif = [trimmed_mean(AoI_unif_all[L]) for L in Lambda_list]
    tm_full = [trimmed_mean(AoI_full_all[L]) for L in Lambda_list]
    ...
    try:
        ...
        width = 0.25
        plt.figure()
        plt.bar(x - width, tm_inc,  width, label='Incremental')
        plt.bar(x,          tm_unif, width, label='Uniform')
        plt.bar(x + width, tm_full, width, label='PSO+SLSQP')
        plt.xticks(x, [str(int(L)) for L in Lambda_list])
        plt.xlabel(r'$\Lambda_{\max}$')
        plt.ylabel('Average AoI (trimmed: drop top/bottom 5)')
        plt.legend()
        plt.tight_layout()
        plt.show()
    except ImportError:
