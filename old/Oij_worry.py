...
def spawn_walkers(world, blueprints, center):
    ...
    for i in range(NUM_WALKERS):
        ...
        walkers.append(w); ctrls.append(c)
        actors += [w, c]

    world.tick()
    for c in ctrls:
        c.start(); c.set_max_speed(WALKER_MAX_SPEED)
        tgt = random_point_in_disk(center, SPAWN_RADIUS * 0.95) if USE_DISK_TARGETS else world.get_random_location_from_navigation()
        if tgt: c.go_to_location(tgt)
...
def main():
    ...
    # --- CLEAN UP EXISTING WALKERS & CONTROLLERS ---
    for a in world.get_actors().filter('walker.pedestrian.*'):
        a.destroy()
    for a in world.get_actors().filter('controller.ai.walker'):
        a.destroy()

    # optional map load (remove if unused)
    # TOWN_NAME = None
    # if TOWN_NAME and world.get_map().name != TOWN_NAME:
    ...
    cameras.append(("fixed_camera", fixed_cam)); actor_list.append(fixed_cam)

    # walker cameras
    if ENABLE_WALKER_CAMERAS:
        for i, w in enumerate(walkers):
            tf = carla.Transform(carla.Location(0,0,1.7))
            c = world.spawn_actor(cam_bp, tf, attach_to=w)
            cameras.append((f"walker_{i}_camera", c)); actor_list.append(c)
    ...
    for frame in range(TOTAL_FRAMES):
        ...
        for name, cam in cameras:
            ...
            labels  = []
            for w in walkers:
                u, v, front = project_world_point(w.get_location(), M_wc, K)
                if front and in_image(u, v, IMAGE_WIDTH, IMAGE_HEIGHT):
                    vis_ids.append(w.id)
    ...
    print(f"[INFO] Completed {TOTAL_FRAMES} frames in {total:.2f}s (avg {total/TOTAL_FRAMES*1000:.1f} ms/frame)")
