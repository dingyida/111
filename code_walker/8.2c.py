...
class IncrementalOptimizer:
    ...
    def update(self, O_mat, g):
        ...
        # Proxy incremental: reoptimize via hybrid on current O_mat
...
def main():
    ...
    # fixed mu and g_const seeds to be repeatable
    np.random.seed(42)
    ...
    for idx, (fp, t) in enumerate(zip(files, frames)):
        ...
        # compute empirical AoIs with distinct seeds
        emp_inc = simulate_and_empirical(lam_inc, mu, O_mat, g_const, T=50000.0, seed=100 + idx)
        emp_prop = simulate_and_empirical(lam_prop, mu, O_mat, g_const, T=50000.0, seed=1000 + idx)
        emp_full = simulate_and_empirical(lam_full, mu, O_mat, g_const, T=50000.0, seed=5000 + idx)

        emp_aoi["incremental"][idx] = emp_inc["emp_mean"]
        emp_aoi["proportional"][idx] = emp_prop["emp_mean"]
        emp_aoi["full"][idx] = emp_full["emp_mean"]

        # theoretical AoI only for incremental
