...
# ================== PRINT HELPERS ==================
def print_lam_named(label, vec, names):
    pairs = ", ".join(f"{n}:{v:.4f}" for n, v in zip(names, vec))
    print(f"    {label:<13} {pairs} | sum={vec.sum():.4f}")
...
def theoretical_quantities(lam, mu, O_mat, g):
    ...
    b = pii0 + pii1
    num = (b[:, None] * O_mat).sum(axis=0)
    # den_j = Σ_i b_i μ_i O_{ij}
    den = ((b * mu)[:, None] * O_mat).sum(axis=0) + EPS
    Dproc = num / den  # shape (J,)

    # (optional) the weights w_i^(j) if you want them:
    # w = ((b * mu)[:, None] * O_mat) / den[None, :]

    # ---------- D_j^Wait (same for all j) ----------

    Dwait_scalar = np.sum(b / (mu + L + EPS))  # 单个标量，对所有 j 都相同
    Dwait = np.full_like(Dproc, Dwait_scalar)
    Dj = Dwait + Dproc
...
def hybrid_opt(obj, mu, O_mat, g, top_k=10):
    ...
    for i in idxs:
        res = minimize(obj, pbest[i], args=(mu, O_mat, g),
                       ...
                       constraints=[cons],
                       options={'ftol':1e-12, 'maxiter':slsqp_max_iter})
...
class IncrementalOptimizer:
    ...
    def update(self, O_mat, g):
        ...
        def obj(lam_local):
            th = theoretical_quantities(lam_local, self.mu, O_mat, g)
            return float(np.mean(th["AoIvec"]))

        bounds = [(EPS, None)] * len(self.mu)
        cons = {'type': 'ineq', 'fun': lambda lam: self.Lambda_max - np.sum(lam)}
...
def main():
    ...
    theo_inc_aoi2 = np.zeros(n_frames)
    ...
    for idx, (fp, t) in enumerate(zip(files, frames)):
        ...
        # ---- print λ allocations for this frame ----
        print(f"[Frame {t}] λ allocations:")
        print_lam_named("incremental:",  lam_inc,  cams)
        print_lam_named("proportional:", lam_prop, cams)
        print_lam_named("full:",         lam_full, cams)
        ...
        emp_full = simulate_and_empirical(lam_full, mu, O_mat, g_const, T=4800.0, seed=100 + idx)

        emp_aoi["incremental"][idx] = emp_inc["emp_mean"]
        emp_aoi["proportional"][idx] = emp_prop["emp_mean"]
        emp_aoi["full"][idx] = emp_full["emp_mean"]

        th_full = theoretical_quantities(lam_full, mu, O_mat, g_const)
        th_SLSQP = theoretical_quantities(lam_inc, mu, O_mat, g_const)
        theo_inc_aoi[idx] = np.mean(th_full["AoIvec"])
        theo_inc_aoi2[idx] = np.mean(th_SLSQP["AoIvec"])
        print(f"[Frame {t}] Empirical AoI: inc={emp_aoi['incremental'][idx]:.4f}, "
              f"prop={emp_aoi['proportional'][idx]:.4f}, full={emp_aoi['full'][idx]:.4f}; "
              f"Theo-inc={theo_inc_aoi[idx]:.4f}; Theo-SLSQP={theo_inc_aoi2[idx]:.4f}")

    # --- compute & print overall averages ---
    avg_inc      = np.mean(emp_aoi["incremental"])
    avg_prop     = np.mean(emp_aoi["proportional"])
    avg_full     = np.mean(emp_aoi["full"])
    avg_theo_inc = np.mean(theo_inc_aoi)
    print(
        f"\nAverage Empirical AoI → inc={avg_inc:.4f}, prop={avg_prop:.4f}, "
        f"full={avg_full:.4f}; Theo-inc={avg_theo_inc:.4f}"
    )

    # ---- optional: per-camera mean λ across frames ----
    print("\nMean λ per camera across frames:")
    print_lam_named("inc-mean:",  lam_store["incremental"].mean(axis=0), cams)
    print_lam_named("prop-mean:", lam_store["proportional"].mean(axis=0), cams)
    print_lam_named("full-mean:", lam_store["full"].mean(axis=0), cams)

    # Save results (note: dicts saved as object arrays)
